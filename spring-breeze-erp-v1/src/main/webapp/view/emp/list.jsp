	...
	<section class="container card my-3">
		<div class="container card my-3 p-3">
			<form action="${pageContext.request.contextPath}/emp/list"
				...
				<div class="col-auto">
					...
					<select class="form-select" id="deptId" name="deptId">
						...
						<c:forEach var="dept" items="${deptList}">
							<option value="${dept.deptId}" <c:if test="${search.deptId == dept.deptId}">selected </c:if>>
									${dept.deptName}
							</option>
						</c:forEach>
					</select>
				...
				<div class="col-auto">
					...
					<select id="posId" name="posId" class="form-select">
						...
						<c:forEach var="pos" items="${posList}">
							<option value="${pos.posId}" <c:if test="${search.posId == pos.posId}">selected</c:if>>
								${pos.posName}
							</option>
						</c:forEach>
					</select>
				...
				<div class="col-auto">
					<label for="empStatus">재직 상태</label> <select id="empStatus"
								...
								<tr>
									...
									<td>
										<a
											...
											title="상세정보 보기" class="btn btn-light">상세</a>
										<a
											href="${pageContext.request.contextPath}/emp/edit?empId=${dto.empId}"
											title="정보 수정하기" class="btn btn-primary">수정</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
