...
public class CompanyService_JPA {

	private final CompanyRepository repo;
	
	// 회사 등록
	@Transactional
	public ComResponse createCompany(ComRequest requestDto) {
		...
		return new ComResponse(savedCom);
	}

	// 회사 단건 조회
	public ComResponse getCompany(Long id) {
		Company com = repo.findById(id)
				          .orElseThrow(()-> new IllegalArgumentException("존재하지 않는 회사입니다. ID="+id));
		return new ComResponse(com);
	}

	// 회사 목록 조회 (검색 + 페이징)
	public List<ComResponse> getAllCompanies(CompanySearchRequest search) {
	...
	// 회사 수정
	@Transactional
	public ComResponse updateCompany(@PathVariable("id") Long id, ComRequest requestDto) {
		Company com = repo.findById(id)
		          .orElseThrow(()-> new IllegalArgumentException("존재하지 않는 회사입니다. ID="+id));
	...
	public List<CompanySearchRequest> getSuggest(String keyword) {
		return repo.findTop5ByComNameContainingOrderByComNameAsc(keyword).stream()
				.map(com -> new CompanySearchRequest(com.getComId().intValue(), com.getComName()))
				.collect(Collectors.toList());
	}

	public StatsComResponse getStats() {
		...
		return new StatsComResponse(comTotal, empTotal, industTotal, comLatest);
	}
