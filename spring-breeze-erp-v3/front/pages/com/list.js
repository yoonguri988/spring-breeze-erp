...
import { Row, Col, Card, Table, AutoComplete, Select, Button, Pagination, Modal, Form, Input, Tag, Avatar, message, } from "antd";
import { PlusOutlined, SearchOutlined, EyeOutlined, EditOutlined, ApartmentOutlined, DeleteOutlined, BankOutlined, TeamOutlined, AppstoreOutlined, CalendarOutlined, } from "@ant-design/icons";
...
export default function ComListPage() {
    ...
    const { list, listTotal, stats, suggestList, 
            loading, error, success, message: companyMessage, } = useSelector((state) => state.company);
...
// 삭제 모달 (companyReducer 에는 모달 상태가 없으므로 페이지 로컬 상태로 관리)
  ...
  useEffect(() => {
    ...
    const { keyword: kw = "", industryGrpCode = "", pstartno = "1", onepagelist = "10" } =
      router.query;
    const next = {
      keyword: kw,
      industryGrpCode,
  ...
  const handleIndustryChange = (value) => pushQuery({ industryGrpCode: value, pstartno: 1 });
  const handlePageSizeChange = (value) => pushQuery({ onepagelist: value, pstartno: 1 });
  ...
  const columns = [
    {
      ...
      key: "no",
      width: 60,
      render: (_, __, idx) => listTotal - (filters.pstartno - 1) * filters.onepagelist - idx,
    },
    {
      title: "회사명",
      dataIndex: "comName",
      ...
      render: (name, record) => (
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Avatar shape="square" src={record.comLogo || undefined} icon={!record.comLogo && <BankOutlined />} />
    ...
    { title: "사업자번호", dataIndex: "bizNo", key: "bizNo", width: 140 },
    {
      ...
      key: "industryGrpCode",
      render: (code) =>
        code ? <Tag>{INDUSTRY_GRP_MAP[code] || "기타"}</Tag> : <span style={{ color: "#bbb" }}>-</span>,
    },
    { title: "임직원 수", dataIndex: "empCount", key: "empCount", width: 110, align: "right" },
    {
      title: "관리",
      key: "actions",
      width: 170,
      ...
      render: (_, record) => (
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 4 }}>
          <Link href={{ pathname: "/com/detail", query: { comId: record.comId } }}>
            <Button type="text" size="small" icon={<EyeOutlined />} title="상세보기" />
          </Link>
          <Link href={{ pathname: "/com/edit", query: { comId: record.comId } }}>
            <Button type="text" size="small" icon={<EditOutlined />} title="수정" />
          </Link>
          <Link href={{ pathname: "/dept/list", query: { comId: record.comId } }}>
            <Button type="text" size="small" icon={<ApartmentOutlined />} title="조직도 보기" />
          </Link>
          <Button
            type="text"
            size="small"
    ...
    <div className="sb-page">
      ...
      <div className="sb-page-head" style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        <div className="sb-page-head__txt">
          <div className="sb-breadcrumb">조직 관리 &gt; 회사 관리 &gt; 목록</div>
          <h1>회사 관리</h1>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col xs={12} lg={6}>
          <StatTile icon={<BankOutlined />} tone="blue" label="전체 회사" value={stats?.comTotal} />
        </Col>
        <Col xs={12} lg={6}>
          <StatTile icon={<TeamOutlined />} tone="green" label="전체 임직원" value={stats?.empTotal} />
        </Col>
        <Col xs={12} lg={6}>
          <StatTile icon={<AppstoreOutlined />} tone="violet" label="업종 수" value={stats?.industTotal} />
        </Col>
        <Col xs={12} lg={6}>
          <StatTile icon={<CalendarOutlined />} tone="amber" label="최근 등록 회사" value={stats?.comLatest} />
        </Col>
      </Row>
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          ...
          {filters.keyword ? (
            <Button onClick={() => pushQuery({ keyword: "", pstartno: 1 })}>초기화</Button>
          ) : null}
        </div>
      ...
      >
        <p>
          정말로 <b>{deleteTarget?.comName}</b> 회사를 삭제하시겠습니까? 본인 확인을 위해 비밀번호를 입력해주세요.
        </p>
