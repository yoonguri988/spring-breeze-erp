...
describe("auth saga", () => {
    ...
    it("login success", () => {
        ...
        const mockResponse = { data: { accessToken: "token-abc", empId: 1, comId: 10 } };
        const putStep = generator.next(mockResponse).value;

        // 3단계: 성공 액션 디스패치 확인
        expect(putStep).toEqual(
            put(loginSuccess({ user: { empId: 1, comId: 10 }, accessToken: "token-abc" }))
        );
    ...
    it("login failure - accessToken이 없는 응답", () => {
        ...
        expect(putStep).toEqual(put(loginFailure("로그인 응답이 올바르지 않습니다.")));
    ...
    it("login failure - API 에러 응답", () => {
        ...
        const error = { response: { data: { error: "이메일 또는 비밀번호가 올바르지 않습니다." } } };
        const putStep = generator.throw(error).value; // catch 블록으로 진입

        expect(putStep).toEqual(put(loginFailure("이메일 또는 비밀번호가 올바르지 않습니다.")));
        expect(generator.next().done).toBe(true);
    });
    ...
    it("refreshToken success", () => {
        ...
        expect(putStep).toEqual(put(refreshTokenSuccess({ accessToken: "new-token" })));
        expect(generator.next().done).toBe(true);
    });
    ...
    it("confirm success (state: OK)", () => {
        ...
        const mockResponse = { data: { state: "OK", resetToken: "reset-token-xyz" } };
        const putStep = generator.next(mockResponse).value;
    ...
    it("updatePass failure - resetToken은 그대로 유지", () => {
        ...
        const error = { response: { data: { error: "유효하지 않거나 만료된 요청입니다." } } };
        const putStep = generator.throw(error).value;

        expect(putStep).toEqual(put(updatePassFailure("유효하지 않거나 만료된 요청입니다.")));
        expect(window.sessionStorage.getItem("resetToken")).toBe("stored-reset-token");
    });
