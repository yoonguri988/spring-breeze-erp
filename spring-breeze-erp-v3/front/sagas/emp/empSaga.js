...
export const updateEmpApi = ({ empId, ...data }) =>
  api.put(`${EMP_API_BASE}/${empId}`, data);
...
export const updatePasswordApi = ({ empId, ...data }) =>
  api.put(`${EMP_API_BASE}/${empId}/password`, data);
...
export function* updatePassword(action) {
  try {
    ...
    yield put(
      updatePasswordFailure(err.response?.data?.message || err.message),
    );
  }
}
...
export const resetPasswordApi = (empId) =>
  api.put(`${EMP_API_BASE}/${empId}/reset-password`);
...
export const checkEmailApi = (email) =>
  api.get(`${EMP_API_BASE}/check-email`, { params: { email } });
...
export const checkMobileApi = (mobile) =>
  api.get(`${EMP_API_BASE}/check-mobile`, { params: { mobile } });
...
export const checkEmpNoApi = (empNo) =>
  api.get(`${EMP_API_BASE}/check-empno`, { params: { empNo } });
...
function* watchListEmp() {
  yield takeLatest(listEmpRequest.type, listEmp);
}
function* watchDetailEmp() {
  yield takeLatest(detailEmpRequest.type, detailEmp);
}
function* watchCreateEmp() {
  yield takeLatest(createEmpRequest.type, createEmp);
}
function* watchUpdateEmp() {
  yield takeLatest(updateEmpRequest.type, updateEmp);
}
...
function* watchCheckEmail() {
  yield takeLatest(checkEmailRequest.type, checkEmail);
}
function* watchCheckMobile() {
  yield takeLatest(checkMobileRequest.type, checkMobile);
}
function* watchCheckEmpNo() {
  yield takeLatest(checkEmpNoRequest.type, checkEmpNo);
}
