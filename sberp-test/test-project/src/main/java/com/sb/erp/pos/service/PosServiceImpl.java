...
public class PosServiceImpl implements PosService {
	...
	public List<PosResponse> selectAll() {
		return posMapper.selectAll(SecurityUtil.getCurrentComId());
	}

	@Override
	public PosResponse selectOneById(long posId) {
		// comId를 함께 넘겨 타 회사 직급 조회를 차단
		return posMapper.selectOneById(posId, SecurityUtil.getCurrentComId());
	}
	...
	public boolean isPosCodeDuplicate(String posCode, Long excludePosId) {
		...
		return posMapper.countByPosCode(posCode, SecurityUtil.getCurrentComId(), excludePosId) > 0;
	}
}
