...
public interface EvalService {
	...
	List<EvalResponse> selectTargetsByCurrentEvaluator(long periodId);
	...
	List<EvalResponse> selectMyEvalHistory(); // 본인
	List<EvalResponse> selectEvalHistoryByEmpId(long empId); // 관리자 조회용


	// ─── 통계 ────────────────────────────────

	// 회차 내 나(로그인 사용자)의 평가 진행률
	int countMySubmitted(long periodId);
	...
	int saveDraft(EvalRequest dto);

	// 최종 제출 (모든 점수 필수)
	// 반환값: 1=성공, -1=회차/대상 없음, -2=회차가 OPEN 아님, -3=평가자 아님, -4=점수/코멘트 누락
	int submit(EvalRequest dto);
}
